...
export default function ControlPanel({ onCompareClick, isCompareOpen, fleet, setFleet, searchQuery, setSearchQuery }: ControlPanelProps) {
    ...
    return (
        <div className="w-full flex items-center justify-between px-6 py-3 border-b border-border relative">
            ...
            {!isCompareOpen && (
                <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center gap-2 bg-background rounded-full border border-border z-10" ref={dropdownRef}>
                    {/* Fleet label pill */}
            ...
            {/* Right: Search Bar */}
            <div className="flex items-center bg-dark rounded-full px-5 py-1.5 bg-card/10 backdrop-blur-xs border border-border/64 ">
